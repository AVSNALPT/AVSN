...
const unsigned int MIN_SERVO_ANGLE_PAN = 50;
const unsigned int MAX_SERVO_ANGLE_PAN = 130;
const unsigned int MIN_SERVO_ANGLE_TILT = 45;
const unsigned int MAX_SERVO_ANGLE_TILT = 125;
...
if (servo == SERVO_PAN){
  if (position < MIN_SERVO_ANGLE_PAN) {
    targetPosition = MIN_SERVO_ANGLE_PAN;
  } else if (position > MAX_SERVO_ANGLE_PAN) {
    targetPosition = MAX_SERVO_ANGLE_PAN;
  } else {
    targetPosition = position;
  }
} else {
  if (position < MIN_SERVO_ANGLE_TILT) {
    targetPosition = MIN_SERVO_ANGLE_TILT;
  } else if (position > MAX_SERVO_ANGLE_TILT) {
    targetPosition = MAX_SERVO_ANGLE_TILT;
  } else {
    targetPosition = position;
  }
}
